...
/* ----------------------------------------------------------------------------------------------- */
/*  инициализация начального состояния (start) генератора xorshoft и xorshift* и 
    начального состояния (start и start_2) генератора xorshoft+
...
/*  Функция использует для генерации случайного значения заранее инициализированное начальное
    ...
    @return Функция возвращает сгенерированное случайное число размером 8 байт (64 бита)           */
/* ----------------------------------------------------------------------------------------------- */
 ak_uint64 ak_xorshiftplus( void )
 {
...
/*  Функция использует для генерации случайного значения заранее инициализированное начальное
    ...
    @return Функция возвращает сгенерированное случайное число размером 8 байт (64 бита)           */
/* ----------------------------------------------------------------------------------------------- */
 ak_uint64 ak_xorshiftstar( void )
 {
     ak_uint64 help = start;
     help ^= help >> 12;
     help ^= help << 25;
     help ^= help >> 27;
     start = help;
     return help * 0x2545F4914F6CDD1D;
 }
